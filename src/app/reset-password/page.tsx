    ...
    (async () => {
      try {
        ...
        if (data.session) setStatus("ready");
          ...
          setStatus("error");
          setMsg("This reset link is invalid or has expired. Go back to sign in and tap “Forgot password” again.");
        }
      } catch (e) {
        setStatus("error");
  ...
  return (
    <div className="fixed inset-0 z-[500] bg-[#1a1a2e] flex items-center justify-center p-5">
      <div className="w-full max-w-[380px] text-center">
        ...
        {status === "loading" && <p className="text-white/60 text-sm">Checking your reset link…</p>}

        {status === "error" && <p className="text-[#f87171] text-sm">{msg}</p>}

        {status === "ready" && (
          <form onSubmit={handleSubmit}>
            <p className="text-white/70 text-sm mb-5">Choose a new password for your account.</p>
            <input
              type="password"
